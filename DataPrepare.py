...
randomSeed = 448
#--------------------------------------
InputRoot = '../DATASET' # input data folder
projectroot = '../ProjectFolder' # project folder
IfNorm = True
#%% 
...
class classificationDataPrepare(object):
    ...
    def readInput(self, checkItem, randomSeed=None):
        for key in self.classList:
            self.FilesInDiffClassDict[key] = list()
            for i in range(len(self.inputDict[key])):
                self.FilesInDiffClassDict[key].extend(glob.glob(os.path.join(self.inputDict[key][i],checkItem)))
    ...
    def GetRawData(self, IfNorm=True):
        for key in self.classList:
            ...
            for item in itemList:
                itemInfo = dict()
                itemInfo['IPD'] = self.__GetPID(item)
                print(item)
...
if __name__ =='__main__':
    ...
    inputDict = dict()
    inputDict['MSA'] = list()
    inputDict['MSA'].append(os.path.abspath(os.path.join(InputRoot, 'MSA')))  
    inputDict['IPD'] = list()
    inputDict['IPD'].append(os.path.abspath(os.path.join(InputRoot, 'IPD')))
    inputDict['PSP'] = list()
    inputDict['PSP'].append(os.path.abspath(os.path.join(InputRoot, 'PSP')))
    inputDict['NC'] = list()
    inputDict['NC'].append(os.path.abspath(os.path.join(InputRoot, 'NC')))

    labelDef = config['labelDef']

    outputFolder = os.path.abspath(os.path.join(projectroot,'data_{0}_{1}'.format(kfold,currentFold)))

    if not os.path.exists(outputFolder):
        os.makedirs(outputFolder)
    dataPrepare = classificationDataPrepare(inputDict, labelDef, outputFolder)
    dataPrepare.readInput(checkItem=checkItem)
    dataPrepare.GetRawData()
    dataPrepare.Train_Test_rawData_Kfold(kfold, currentFold)     